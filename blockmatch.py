...
def block_match(tgt, src, tile_size=16, tile_step=16, max_disp=10,
                min_overlap_px=500, min_overlap_ratio=0.2,
                div_by_overlap=True,
    ...
    for x_tile in range(0, tile_count):
        for y_tile in range(0, tile_count):
            ...
            if get_black_fraction(src_tile, 0) > 0.3 or get_black_fraction(tgt_tile, 0) > 0.9:
                ...
                ncc = get_ncc(tgt_tile, src_tile, div_by_overlap=div_by_overlap,
...
def get_ncc(tgt, tmpl, div_by_overlap=False, min_overlap_ratio=0.1,
