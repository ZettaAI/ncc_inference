...
def misalignment_detector(img1, img2, mip, np_out=True, threshold=None):
    ...
    img1_downs_norm = normalize(img1_downs, mask=img1_downs.abs()>0.05, mask_fill=-20)
    img2_downs_norm = normalize(img2_downs, mask=img2_downs.abs()>0.05, mask_fill=-20)
    ...
    return get_np(misalignment_mask)
...
def compute_fcorr(image1, image2):
    ...
    bm_result = block_match(image2, image1, min_overlap_px=4000, tile_step=tile_size//2,
            tile_size=tile_size,
            peak_ratio_cutoff=6.5, peak_distance=ma_length,  max_disp=16, filler=250)
